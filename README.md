...
- Erişim anahtarı (token) yönetimi: DB'den veya ortam değişkeninden. SHA-256 hash + plaintext olarak saklanır, admin panelinde tam metin gösterilir.
...
- Brute-force koruması: yönetim panelinde IP bazlı kilitleme, üstel geri çekilme (exponential backoff).
- Public istatistik sayfası: toplam istek, günlük istek, aktif kullanıcı sayısı, saatlik grafik, en çok kullanılan modeller (`/stats`).
- Hata dönüşümü: NVIDIA context/token aşımı → Anthropic `invalid_request_error` (istemci context compression tetikler), multimodal desteklenmeyen model → açıklayıcı hata, model bulunamadı → `not_found`.
- Model benchmark sistemi: seçilen modellerin yanıt sürelerini (TTFB + toplam) saatlik olarak ölçer, sonuçları ana sayfada yayınlar. Admin panelden hangi modeller için aktif olacağı seçilir.
- SSE keepalive: 15 saniyede bir keepalive mesajı gönderir, reverse proxy idle timeout'ları engeller.
...
### RunCodingPlan ile (Önerilen)

```bash
npx runcodingplan --add-custom --name "Anthronim" --url "http://localhost:8787" -a <erişim anahtarınız> -m minimaxai/minimax-m2.5
```

Başlatmak için:

```bash
npx runcodingplan -p anthronim
```

Diğer modelleri eklemek için:

```bash
npx runcodingplan -p anthronim --add-model minimaxai/minimax-m2.7
```

Detaylar: [github.com/ersinkoc/RunCodingPlan](https://github.com/ersinkoc/RunCodingPlan)
...
| `TZ`                       | Hayır   | `Europe/Istanbul`                     | Saat dilimi (SQLite `localtime`, Docker `tzdata` gerektirir)    |
...
| `BENCH_INTERVAL_MINUTES`   | Hayır   | `60`                                  | Benchmark çalışma aralığı (dakika)                              |

## Uç Noktalar

| Yöntem  | Yol                     | Davranış                                                                    |
|---------|-------------------------|-----------------------------------------------------------------------------|
| GET     | `/`                     | Public landing sayfası (istatistikler, benchmark sonuçları, yapılandırma, model listesi) |
...
| GET     | `/stats`                | Public istatistikler (toplam istek, bugün, aktif kullanıcı, saatlik, model) |
...
| GET     | `/admin/api/tokens`     | Erişim anahtarı listesi (tam plaintext)                                     |
| POST    | `/admin/api/tokens`     | Yeni erişim anahtarı ekle `{ "token": "...", "label": "..." }`              |
| PATCH   | `/admin/api/tokens/:id` | Düzenle: `{ "isActive": bool }`, `{ "label": "..." }` veya `{ "token": "..." }` |
| DELETE  | `/admin/api/tokens/:id` | Erişim anahtarını sil                                                       |
| GET     | `/admin/api/models`     | NVIDIA model kataloğu (admin cache)                                         |
| GET     | `/admin/api/benchmarks` | Benchmark model listesi                                                     |
| POST    | `/admin/api/benchmarks` | Benchmark modeli ekle `{ "model": "..." }`                                  |
| PATCH   | `/admin/api/benchmarks/:id` | Benchmark modelini etkinleştir/devre dışı bırak `{ "isActive": true/false }` |
| DELETE  | `/admin/api/benchmarks/:id` | Benchmark modelini sil                                                    |
...
- Erişim anahtarları veritabanında SHA-256 hash olarak saklanır (plaintext sütunu admin gösterimi için).
- Yönetim paneli brute-force korumalıdır (IP bazlı kilitleme, üstel geri çekilme).
...
- Erişim anahtarı (token) yönetimi (ekleme, düzenleme, silme, etkinleştirme/devre dışı bırakma, otomatik üretim)
- Upstream hata log görüntüleyici (`/admin/logs`)
- Saatlik istek grafiği (son 24 saat)
- Model bazlı kullanım dağılımı
- Benchmark model yönetimi (ekleme, silme, etkinleştirme/devre dışı bırakma)
